...
class DataModule:
    ...
    def _init_datasets(self):
        ...
        a_train_idx = ds_a_full.indices_from_txt(cfg.train_txt) #both are in text files, text files are common for A & D
        ...
        #C is from completely different dataset with completely different images, just for testing
        self.ds_c_test = DatasetC(cfg.root_c, "test", cfg.max_size, self.num_channels)

        n_val_a = max(1, int(len(ds_a_train_full) * self.conf.val_frac)) #there is only train and test no validation
        n_val_b = max(1, int(len(ds_b_full) * self.conf.val_frac))
        g = torch.Generator().manual_seed(42)

        #Split A & B Train into Train & Val (Split is random we don't really care for training, Test is separate)
        ...
        # Custom small instance seg dataset
        self.ds_custom = DatasetCustom(cfg.root_custom, cfg.max_size, self.num_channels)

        #I think A & D actually have the same images but different annotation so we only need the Test section because it has structural abnormalities listed
    ...
    #Training loaders split into TRAIN & VAL
    def make_loaders_a(self) -> Tuple[DataLoader, DataLoader]:
        train_loader = DataLoader(self.ds_a_train, batch_size=self.conf.batch_size, shuffle=True, num_workers=self.conf.num_workers, collate_fn=collate_bb)
        val_loader = DataLoader(self.ds_a_val, batch_size=self.conf.batch_size, shuffle=False, num_workers=self.conf.num_workers, collate_fn=collate_bb)
    ...
    def make_loader_custom(self) -> DataLoader:
        return DataLoader(self.ds_custom, batch_size=1, shuffle=False, num_workers=self.conf.num_workers, collate_fn=collate_bb)
